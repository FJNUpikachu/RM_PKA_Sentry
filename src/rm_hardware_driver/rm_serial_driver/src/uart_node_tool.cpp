...
#include "rm_interfaces/msg/current_hp.hpp"
  ...
  try {
    ...
    if (node->debug_) {
      PKA_DEBUG("serial_node",
        ...
        "vx={:.4f} vy={:.4f} wz={:.4f} pose_status={}",
        bytes_wrote, packed.size(),
        data.fire_advice, data.pitch, data.yaw, data.distance,
        data.linear_x, data.linear_y, data.angular_z, data.pose_status);
...
{
  {
    rm_interfaces::msg::GameProgress gp;
    gp.game_progress = recv_msg.game_progress;
    node->game_progress_pub_->publish(gp);
  }
  {
    rm_interfaces::msg::CurrentHP hp;
    hp.current_hp = recv_msg.current_hp;
    node->current_hp_pub_->publish(hp);
  }
  {
    rm_interfaces::msg::BulletRest br;
    br.bullet_rest = recv_msg.bullet_rest;
    node->bullet_rest_pub_->publish(br);
  }
  {
    rm_interfaces::msg::IsBattling ib;
    ib.is_battling = recv_msg.is_battling;
    node->is_battling_pub_->publish(ib);
  }
  {
    rm_interfaces::msg::IsFortress ifo;
    ifo.is_fortress = recv_msg.is_fortress;
    node->is_fortress_pub_->publish(ifo);
  }
  {
    rm_interfaces::msg::IsSupply isp;
    isp.is_supply = recv_msg.is_supply;
    node->is_supply_pub_->publish(isp);
  }
  {
    rm_interfaces::msg::OutpostHP oh;
    oh.outpost_hp = recv_msg.outpost_hp;
    node->outpost_hp_pub_->publish(oh);
  }
}
...
void UARTNodeTool::mock_recv_timer_cb(UARTNode * node)
{
  switch (node->robot_type_) {
    ...
    case RobotType::SENTRY: {
      ...
      msg.is_fortress            = 0;
      msg.is_supply              = 0;
      msg.outpost_hp             = node->mock_outpost_hp_;
...
void UARTNodeTool::virtual_send_timer_cb(UARTNode * node)
{
  switch (node->robot_type_) {
    ...
    case RobotType::SENTRY: {
      ...
      vd.pose_status  = node->virtual_pose_status_;

      if (node->debug_) {
        PKA_DEBUG("serial_node",
          "[virtual_send/sentry] fire={} pitch={:.4f} yaw={:.4f} dist={:.4f} "
          "vx={:.4f} vy={:.4f} wz={:.4f} pose={}",
          vd.fire_advice, vd.pitch, vd.yaw, vd.distance,
          vd.linear_x, vd.linear_y, vd.angular_z, vd.pose_status);
      }
