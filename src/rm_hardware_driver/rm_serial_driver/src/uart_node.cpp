...
void UARTNode::init_parameters()
{
  ...
  mock_outpost_hp_             = static_cast<uint16_t>(declare_parameter("mock_outpost_hp", 0));
  ...
  virtual_pose_status_  = static_cast<uint8_t>(
    declare_parameter("virtual_pose_status", 0));
...
void UARTNode::init_subscriber()
{
  ...
  if (robot_type_ == RobotType::SENTRY && !pose_status_topic_.empty()) {
...
void UARTNode::init_publisher()
{
  ...
  switch (robot_type_) {
    case RobotType::SENTRY:
      ...
      game_progress_pub_ = create_publisher<rm_interfaces::msg::GameProgress>(
        "/game_progress", 10);
      current_hp_pub_    = create_publisher<rm_interfaces::msg::CurrentHP>(
        "/current_hp", 10);
      bullet_rest_pub_   = create_publisher<rm_interfaces::msg::BulletRest>(
        "/bullet_rest", 10);
      is_battling_pub_   = create_publisher<rm_interfaces::msg::IsBattling>(
        "/is_battling", 10);
      is_fortress_pub_   = create_publisher<rm_interfaces::msg::IsFortress>(
        "/is_fortress", 10);
      is_supply_pub_     = create_publisher<rm_interfaces::msg::IsSupply>(
        "/is_supply", 10);
      outpost_hp_pub_    = create_publisher<rm_interfaces::msg::OutpostHP>(
        "/outpost_hp", 10);
      PKA_INFO("serial_node",
        "Publishers: serial/receive(sentry), game_progress, current_hp, "
        "bullet_rest, is_battling, is_fortress, is_supply, outpost_hp"
        );
      break;
