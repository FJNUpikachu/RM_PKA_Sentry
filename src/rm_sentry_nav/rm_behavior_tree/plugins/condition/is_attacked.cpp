...
: BT::SimpleConditionNode(name, std::bind(&IsAttackedCondition::checkRobotAttacked, this), config),
  msg_received_(false) // 初始化消息接收标志
{
  ...
  subscriber_ = node_->create_subscription<rm_interfaces::msg::IsAttacked>(
    ...
    std::bind(&IsAttackedCondition::messageCallback, this, std::placeholders::_1)
  );
}

BT::NodeStatus IsAttackedCondition::checkRobotAttacked()
{
  // 检查是否接收到消息
  if (!msg_received_) {
...
void IsAttackedCondition::messageCallback(const rm_interfaces::msg::IsAttacked::SharedPtr msg)
{
...
{
  factory.registerNodeType<rm_behavior_tree::IsAttackedCondition>("IsAttacked");
}
