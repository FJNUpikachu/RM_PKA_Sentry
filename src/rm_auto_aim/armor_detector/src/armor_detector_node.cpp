...
Node("armor_detector", options) {
    ...
    this->odom_frame_ = this->declare_parameter("odom_frame", "odom");
    this->gimbal_frame_ = this->declare_parameter("gimbal_frame", "gimbal_link");
    this->cam_frame_ = this->declare_parameter("cam_frame", "camera_optical_frame");
    ...
    if (this->optimize_yaw) {
        ...
        try {
            ...
            auto R_camera2gimbal_tf = this->tf2_buffer_->lookupTransform(
                this->gimbal_frame_, this->cam_frame_, target_time, rclcpp::Duration::from_seconds(0.05)
            );
            ...
            PKA_ERROR("armor_detector", "TF lookup failed: %s", ex.what());
        ...
        try {
            ...
            PKA_ERROR("armor_detector", "TF lookup failed: {}", ex.what());
            if (this->lookUp_error_count_ > this->lookUp_thres_) rclcpp::shutdown();
            this->lookUp_error_count_++;
            return;
        }
