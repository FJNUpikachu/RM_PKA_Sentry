...
public:
  ...
  */
  ...
  /// Outpost id/display parameters (injected by armor_solver_node).
  OutpostParams outpost_params_{};

  /// Outpost solver parameters (fully independent from ground robot solver).
  /// Injected / refreshed by armor_solver_node via loadOutpostSolverParams().
  OutpostSolverParams outpost_solver_params_{};

  /// 前哨站单板路径用；所有权仍在 Solver，仅暴露裸指针供节点同步参数与调用解算。
  TrajectoryCompensator * outpostTrajectoryCompensator() noexcept
  {
    return outpost_trajectory_compensator_.get();
  }
  ManualCompensator * outpostManualCompensator() noexcept { return outpost_manual_compensator_.get(); }
...
private:
  ...
  // target_v_yaw:      当前目标角速度（rad/s），用于按速度选择 fire_margin
  ...
  // Ground robot compensators
  std::unique_ptr<TrajectoryCompensator> trajectory_compensator_;
  std::unique_ptr<ManualCompensator> manual_compensator_;

  // Outpost-specific compensators (configured from outpost.solver.* params)
  std::unique_ptr<TrajectoryCompensator> outpost_trajectory_compensator_;
  std::unique_ptr<ManualCompensator> outpost_manual_compensator_;
  ...
  // 当目标角速度较大时使用的 fire_margin（由 YAML 注入）
  double fire_margin_fast_;
  // 选择 fire_margin 的角速度阈值（rad/s）
  double fire_margin_v_yaw_threshold_;
  ...
  // 高速（大角速度）时使用的偏移量（当 |v_yaw| > offset_v_yaw_threshold_）
  double yaw_offset_fast_;   // 单位：度
  double pitch_offset_fast_; // 单位：度
  // 选择偏移量的角速度阈值（rad/s），由 YAML 注入
  double offset_v_yaw_threshold_;

  // 基地专用机械偏差（base.yaw_offset / base.pitch_offset，度）；非 BASE 兵种使用 solver 偏置
  double base_yaw_offset_;
  double base_pitch_offset_;
  ...
  // Outpost solver per-frame state (armor lock + fire stability filter)
  OutpostSolverState outpost_solver_state_{};
